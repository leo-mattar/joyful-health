// --------------- GSAP ---------------
gsap.registerPlugin(ScrollTrigger, CustomEase, Draggable, InertiaPlugin, GSDevTools);
...
function testimonialsTabberMobileRevert() {
  ...
  const clonedHeadings = document.querySelectorAll(".c-quote-tab .c-quote-main-title");
...
function storyboardAnimation() {
  ...
  tl.from(".c-img-contain.sb-artwork", {
    duration: 1,
  ...
  tl.to(".c-img-contain.sb-artwork", {
    scale: 0,
    opacity: 0,
    stagger: 0.05
  }, "<");

  tl.to([".c-sb-o", ".c-sb-o-fill"], {
    duration: 1.4,
    scale: 0,
    y: "12em"
  }, ">0.4");

  tl.from(".c-img-contain.sb-graph", {
    duration: 1,
    scale: 0,
    y: "10em",
    borderRadius: "50%",
  ...
  tl.from(".c-img-contain.sb-graph-3", {
    opacity: 0,
    y: "24em"
  }, ">");

  tl.from(".c-img-contain.sb-graph-2", {
    opacity: 0,
    y: "16em"
  }, "<0.3");
  ...
  tl.to(".c-sb-inner > *:not(.c-img-contain.sb-bg)", {
    opacity: 0
  }, ">4");
...
function careersStructure() {
  ...
  teamItems.forEach(function (cmsItem) {
    let itemCategory = cmsItem.querySelector("[data-group-name]").textContent.trim();

    let groupItems = document.querySelectorAll(".c-careers-group-item");
    groupItems.forEach(function (group) {
      let groupCategory = group.querySelector("[data-group-title]").textContent.trim();
...
function buttonHover() {
  $("[data-btn='wrap']:not([data-wf--global-button--settings-style='inline'])").each(function () {
    const clipEl = $(this)
      .find("[data-btn='clip']")
...
function caseTestimonialsMarquee() {
  ...
  const marqueeDuration = window.innerWidth <= 479 ? caseItems * 12 : caseItems * 9;
...
mm.add("(max-width: 991px)", () => {
  ...
  document.querySelector(".c-header-nav").setAttribute("data-lenis-prevent", "");
  if (homepage) {
    homeInfoMobile();
  }
  return () => {
    headerCleanup();
    document.querySelector(".c-header-nav").removeAttribute("data-lenis-prevent", "");
    document.querySelector(".c-btn-group.header").appendChild(document.querySelector(
      ".c-header-cta"));
